...
const YIYUAN_TOKEN = process.env.YIYUAN_TOKEN || 'f5f07a6813d0b25bae66d354feee6775';
const SCAN_DATE_RANGE = process.env.SCAN_DATE_RANGE || `["2024-05-01", "2024-05-05"]`;
...
function queryList () {
  console.log(`扫描区间:${SCAN_DATE_RANGE}`);
  const scanDateRange = JSON.parse(SCAN_DATE_RANGE);
  const beginDate = dayjs(scanDateRange[0]);
  const endDate = dayjs(scanDateRange[1]);
  const now = dayjs();
  // 测试
  ...
  return axios.post(`https://yyzz.zgwhfe.com/KasiteWeb/wsgw/yy/QueryClinicScheduleList/callApi.do?t=${+new Date()}`,
    body,
    {
      ...
      if (RespCode === '10000') {
        ...
        }).filter(x => x.LeaveCount > 0 && dayjs(x.RegDate).diff(beginDate) >= 0 && dayjs(x.RegDate).diff(endDate) <= 0);
        console.log('结果', JSON.stringify(list));
        if (list.length > 0) {
          let first = list[0];
          let times = first.partTime.map(x => `${x.time}(余号:${x.count})`).join('\n')
          let msg = `[${now.format('HH:mm:ss')}]${first.DoctorName}医生在${first.RegDate}${first.TimeSliceStr}有${first.LeaveCount}个可预约号源\n\n${times}\n\n挂号费:￥${first.TotalFee}`;
          console.log(msg);
          // sendMsg('45917290774@chatroom', '', msg);
          sendMsg('47346666379@chatroom', '', msg);
        } else {
          console.log('暂无预约号源');
        }
      } else {
